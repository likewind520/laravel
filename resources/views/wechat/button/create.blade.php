                        ...
                        <div class="mobile">
                            <div class="mobile-container">
                                <dl v-for="(v,k) in menus.button">
                                    {{--一级菜单 start--}}
                                    <dt @click="editCurrentMenu(v)" style="cursor:pointer;">
                                        {{--stop 阻止事件冒泡--}}
                                        <span @click.stop="delTopMenu(k)" class="fa fa-minus-square"  style="cursor:pointer;"></span>
                                        @{{ v.name }}
                                    </dt>
                                    {{--一级菜单 end--}}
                                    {{--二级菜单 start--}}
                                    <dd v-for="(m,n) in v.sub_button" @click="editCurrentMenu(m)" style="cursor:pointer;">
                                        <span @click="delSubMenu(v,n)" class="fa fa-minus-square" style="cursor:pointer;"></span>
                                        @{{ m.name }}
                                    </dd>
                                    {{--二级菜单 end--}}
                                    <dd @click="addSubMenu(v)" v-if="v.sub_button.length<5" style="cursor:pointer;">
                                        <span class="fa fa-plus-square" ></span>
                                    </dd>
                                </dl>
                                <dl v-if="menus.button.length < 3">
                                    <dt @click="addTopMenu"  style="cursor:pointer;">
                                        <span class="fa fa-plus-square" ></span>
                                    </dt>
                                </dl>
                            </div>
                ...
                methods: {
                    ...
                    addTopMenu(){
                        if(this.menus.button.length < 3) {
                            var html = {type: 'click',name: '我是中国人', key: '',sub_button:[]}
                    ...
                    addSubMenu(v){
                        if(v.sub_button.length < 5) {
                            var html = {type: 'click', name: '我是中国人', key: ''}
